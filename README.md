...
- **atomize** — one fix → property + 4 framework adapters + witness + mutation (marauders markers or `patches/<variant>.patch`).
...
- (No per-variant git branches; `patches/*.patch` is the only persisted variant state for patch-kind variants.)
...
Produced workloads live under `workloads/Rust/`. Each has its own `BUGS.md`, `TASKS.md`, `etna.toml`, `src/bin/etna.rs`, and `patches/*.patch` for patch-kind variants.
