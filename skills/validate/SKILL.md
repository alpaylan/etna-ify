...
1. **Base HEAD, debug:** `cargo test witness_<name>_case_` — all witness tests pass.
2. **Base HEAD, release** (witness symmetry under the experiment's actual build profile): for every `Property` mapped from this workload's variants, run `cargo run --release --bin etna -- etna <Property>` and confirm the JSON line has `"status":"passed"` or `"status":"discarded"` — never `"failed"`.
3. **Activate mutation:**
...
4. **Expected, debug:** at least one witness test fails under the mutation.
5. **Expected, release:** the `etna <Property>` invocation returns `"status":"failed"` under the mutation.
