...
Append one `[[variant]]` table:

```toml
[[variant]]
name = "<variant_name>"
property = "<property_name>"                # without the property_ prefix
injection = "marauders"                      # or "patch"
patch = "patches/<variant>.patch"            # only when injection = "patch"
source_commit = "<full_hash>"
witnesses = ["witness_<name>_case_<tag>"]
frameworks = ["proptest", "quickcheck", "crabcheck", "hegel"]
files = ["src/foo.rs", "src/bar.rs"]         # for marauders: where comments live; for patch: where patch applies
```

`etna.toml` is the **only** index we maintain by hand. Every other fact is derivable from source.
...
Terminal reasons only. Record the dropped variant's `name`, `hash`, and reason in a comment in `etna.toml` (not a separate file). Examples:
...
Running `atomize` on the same candidate twice must be safe. Before injecting, check `marauders list` (or `etna.toml`) for the variant name; if present, skip. Before writing a property, grep `pub fn property_<name>`; if it exists and signatures match, reuse it.
